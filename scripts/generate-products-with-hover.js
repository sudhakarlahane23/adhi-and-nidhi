...
categoryFolders.forEach(category => {
    ...
    const mainImages = imageFiles.filter(file => {
    ...
    mainImages.forEach((image, index) => {
        ...
        const hoverFile = imageFiles.find(file => {
        ...
        const hover = hoverFile
            ? `/assets/images/collection/${category}/${hoverFile}`
            : undefined;
        ...
        const slideFiles = imageFiles
            .filter(file => {

                const name = path.basename(file, path.extname(file));

                return name.startsWith(`${filename}-slide-`);
        ...
        const slides = slideFiles.map(file =>
            `/assets/images/collection/${category}/${file}`
        );
        ...
        if (hover) {
            ...
            additionalImages.hover = hover;

        }

        if (slides.length) {

            additionalImages.slides = slides;
